...
# Build and push product-api images (0.9.0 baseline + 1.0.0 for promotion)
echo -e "${YELLOW}Building product-api images...${NC}"
docker build -t "${ECR_URL}/product-api:0.9.0" -t "${ECR_URL}/product-api:1.0.0" ./product-api/

echo -e "${YELLOW}Logging into ECR...${NC}"
aws ecr get-login-password --region "$AWS_REGION" | docker login --username AWS --password-stdin "$ECR_URL"

# Create ECR repo (idempotent)
aws ecr describe-repositories --repository-names product-api --region "$AWS_REGION"  | jq -r 2>/dev/null || \
aws ecr create-repository --repository-name product-api --region "$AWS_REGION" | jq -r

echo -e "${YELLOW}Pushing product-api images to ECR...${NC}"
docker push "${ECR_URL}/product-api:0.9.0"
...
docker pull --platform linux/amd64 bitnamilegacy/redis:7.4.3
docker pull --platform linux/arm64 polinux/stress:latest
kind load docker-image "${ECR_URL}/product-api:0.9.0" --name argo-kargo-demo
kind load docker-image "${ECR_URL}/product-api:1.0.0" --name argo-kargo-demo
kind load docker-image "bitnamilegacy/redis:7.4.3@sha256:fbdf361bbb6a17be28913fb9e4a1cfe3244331d2cbf449ecfe7a1fbbab02efc4" --name argo-kargo-demo
# polinux/stress has a broken multi-platform manifest that corrupts Kind's
# containerd import pipeline. Use docker save + image-archive to isolate it.
docker save polinux/stress:latest -o /tmp/stress.tar
kind load image-archive /tmp/stress.tar --name argo-kargo-demo
rm -f /tmp/stress.tar
...
# repoURL must include the full image path (not just the registry host) for Kargo to match it.
# Using pre-exchanged ECR token (valid 12h) — for longer demos, re-run this block.
...
# Import Grafana dashboard (while password is still admin:admin)
echo -e "${YELLOW}Importing Grafana canary analysis dashboard...${NC}"
for i in {1..15}; do
    if curl -sk http://localhost:3001/api/health >/dev/null 2>&1; then
        curl -s -u admin:admin http://localhost:3001/api/dashboards/db \
            -H 'Content-Type: application/json' \
            -d "{\"dashboard\": $(cat grafana/canary-analysis-dashboard.json), \"overwrite\": true}" >/dev/null 2>&1 \
            && echo -e "${GREEN}Grafana dashboard imported!${NC}" \
            || echo -e "${YELLOW}Dashboard import failed — import grafana/canary-analysis-dashboard.json manually${NC}"
        break
    fi
    sleep 2
done
